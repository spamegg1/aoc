...
direction the keys went if you help one of the
...
returns to the top of the list of actions. For example,
if split produces a pair that meets the explode criteria, that
...
- `[7,[6,[5,[4,[3,2]]]]]` becomes `[7,[6,[5,[7,0]]]]`
  - (2 has no regular number to its right, and so it is not added to any regular number).
...
For example, 10 becomes `[5,5]`, 11 becomes `[5,6]`,
