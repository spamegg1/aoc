...
With the scanners fully deployed, you turn their
attention to mapping the floor of the ocean trench.
...
It is normally given on a single line, but it has been wrapped
to multiple lines in this example for legibility.
The second section is the input image, a two-dimensional grid of
...
(5,11), (6,9), (6,10), and (6,11). These nine input pixels are combined
into a single binary number that is used as an index in the image
...
34 can be found: #. So, the output pixel in the center of the output image
...
Through further advances in imaging technology, the above output
image can also be used as an input image! This allows it to be enhanced a second time:
...
Start again with the original input image and
