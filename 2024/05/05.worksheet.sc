...
In the above example, the first update (75,47,61,53,29) is in the right order:
  75 is correctly first because there are rules
    that put each other page after it: 75|47, 75|61, 75|53, and 75|29.
  47 is correctly second because 75 must be before it (75|47)
    and every other page must be after it according to 47|61, 47|53, and 47|29.
...
number of each correctly-ordered update. In the above example,
...
to put the page numbers in the right order. For the above example,
...
object Solving:
  ...
  def order(rules: Rules)(page: Page): Page = // part 2
    for i <- 0 to page.size - 2 do
      for j <- i + 1 until page.size do
        if rules.contains((page(j), page(i))) then // rule says to swap them
          val temp = page(i)
          page(i) = page(j)
          page(j) = temp
    page
