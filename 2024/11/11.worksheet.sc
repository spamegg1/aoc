...
and while The Historians explore their infinite corridors, you've noticed a
strange set of physics-defying stones.
...
object DataDefs:
  ...
  extension (stones: Stones)
    def freqs(times: Long): Freqs = stones.groupMapReduce(identity)(_ => times)(_ + _)

  extension (freqs: Freqs)
    def blink: Freqs = freqs.toSeq
      .flatMap((stone, times) => stone.blink.freqs(times))
      .groupMapReduce(_._1)(_._2)(_ + _)
      .toMap

object Parsing:
  import DataDefs.*
  def parse(line: String): Stones = line.split(" ").toSeq.map(_.toLong)

object Solving:
  import DataDefs.*

  // this is slow for part 2, but works for part 1
  def solve1(line: String)(times: Int) =
    val startStones = Parsing.parse(line)
    (0 until times)
      .foldLeft(startStones)((stones, _) => stones.flatMap(_.blink))
      .size

  def solve2(line: String)(times: Int) =
    val startFreqs = Parsing.parse(line).freqs(1L)
    (0 until times)
      .foldLeft(startFreqs)((freqs, _) => freqs.blink)
      .values
      .sum
