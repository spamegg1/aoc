...
in the middle of a jungle. The Chief isn't on this side of the
...
stole all the operators from their calibration equations!
...
object DataDefs:
  case class Eqn(res: Long, nums: Seq[Long]):
    def nextPossible(soFar: Seq[Long], next: Long): Seq[Long] =
      soFar.map(_ + next) ++ soFar.map(_ * next)
        ++ soFar.map(n => s"$n$next".toLong) // part 2
    @annotation.tailrec
    private def helper(toDo: Seq[Long], soFar: Seq[Long]): Seq[Long] =
      if toDo.isEmpty then soFar
      else
...
object Parsing:
  ...
  def parseLine(line: String) = line match
    case s"$res: $nums" => Eqn(res.toLong, nums.split(" ").map(_.toLong).toSeq)
