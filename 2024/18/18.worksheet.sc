...
scanning the algorithm, you create a list of which bytes will fall
...
from 0 to 70 both horizontally and vertically. However, for the sake of example,
...
(at 70,70 in your memory space, but at 6,6 in this example).
...
After just 12 bytes have corrupted locations in your memory space,
the shortest path from the top left corner to the exit would take 22 steps.
...
You're afraid they're not going to be fast enough to make it to the exit before
...
So, in this example, the coordinates of the first byte
that prevents the exit from being reachable are 6,1.

Simulate more of the bytes that are about to corrupt your memory space.
What are the coordinates of the first byte that will prevent
...
object DataDefs:
  ...
  type Pos = (row: Int, col: Int) // reversed from problem
  type RAM = MGraph[Pos, UnDiEdge[Pos]]
  ...
  extension (p: Pos)
    ...
    def inBounds(size: Int) = 0 <= p.row && p.row < size && 0 <= p.col && p.col < size
    def possible(size: Int) = p.neighbors.filter(_.inBounds(size))

object Parsing:
  import DataDefs.*
  def parseLine(line: String): Pos = line match
    case s"$col,$row" => (row.toInt, col.toInt)
...
object Solving:
  ...
  def makeGraph(bytes: Seq[Pos])(size: Int): RAM =
    ...
    do
      ...
      pos
        .possible(size)
        .foreach: p =>
          if !bytes.contains(p) then graph += pos ~ p
    graph

  def findPath(graph: RAM)(ramSize: Int) =
    val start = (0, 0)
    val exit  = (ramSize - 1, ramSize - 1)
    graph
      .get(start)
      .shortestPathTo(graph.get(exit))
      .map(_.edges.size)

  def solve1(lines: Seq[String])(ramSize: Int)(bytesSize: Int) =
    val bytes = Parsing.parse(lines)
    val graph = makeGraph(bytes.take(bytesSize))(ramSize)
    findPath(graph)(ramSize)

  def solve2(lines: Seq[String])(ramSize: Int)(startSize: Int): Option[Pos] =
    val bytes = Parsing.parse(lines)
    val graph = makeGraph(bytes.take(startSize))(ramSize)
    val firstFail = (startSize to bytes.size).find: bytesSize =>
      val newByte = bytes(bytesSize)
      graph -= newByte // avoid recreating graph, just mutate instead, big perf. gain
