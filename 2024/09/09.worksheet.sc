...
The disk map uses a dense format to represent the layout of files
and free space on the disk. The digits alternate between indicating
...
just like the amphipod hoped. Second, the computer is running much
...
object DataDefs:
  val FREE = -1L
  type Disk = Array[Long]
  extension (d: Disk)
    def checksum: Long = d.takeWhile(_ != FREE).zipWithIndex.map(_ * _).sum

object Parsing:
  import DataDefs.*
  def parse(line: String): Disk = line.zipWithIndex.toArray
    .flatMap: (size, index) =>
      Array.fill(size.asDigit.toInt):
        if index % 2 == 0 then index.toLong / 2 else FREE
...
object Solving:
  ...
  def solve1(line: String) =
    ...
    while first < last do
      ...
      disk(last) = FREE
      first = disk.indexWhere(_ == FREE, from = first)
      last = disk.lastIndexWhere(_ != FREE, end = last)
  ...
  def solve2(line: String) =
    ...
    // defrag ends if there is no free space to the left of last data block moved
    while disk.indexWhere(_ == FREE) < dataStt do
      // look for large enough free space for data, to the left of the data
      while freeStt < dataStt && freeSize < dataSize do
        freeStt = disk.indexWhere(_ == FREE, from = freeEnd)
        freeEnd = disk.indexWhere(_ != FREE, from = freeStt)
        freeSize = freeEnd - freeStt

      if dataSize <= freeSize then // large enough free space found
        for index <- 0 until dataSize do // swap to defragment
          disk(freeStt + index) = disk(dataEnd - index)
          disk(dataEnd - index) = FREE

      // whether swap happened or not, we move on to next data block
      dataEnd = disk.lastIndexWhere(_ != FREE, end = dataStt)
      dataTyp = disk(dataEnd)
      dataStt = disk.lastIndexWhere(_ != dataTyp, end = dataEnd)

      // start free space search from the beginning for the next data block
      freeStt = disk.indexWhere(_ == FREE, from = 0)
      freeEnd = disk.indexWhere(_ != FREE, from = freeStt)
