...
object Day23:
  ...
  case class Interval(start: Int, end: Int):
    ...
    def manhattan(n: Int): Int =
      if n < start then start - n else if n > end then n - end else 0

  case class Cube(xs: Interval, ys: Interval, zs: Interval):
    def closest: Int = xs.closest + ys.closest + zs.closest
    def split: Seq[Cube] = for nxs <- xs.split; nys <- ys.split; nzs <- zs.split
    yield Cube(nxs, nys, nzs)
    def manhattan(point: Point): Int =
      xs.manhattan(point.x) + ys.manhattan(point.y) + zs.manhattan(point.z)
  ...
  def part2(input: Seq[String]): Int =
    val nanobots = parse(input)
    def score(cube: Cube) =
      cube -> nanobots.count(nb => cube.manhattan(nb.position) <= nb.radius)

    def helper(n: Int, cubes: Seq[Cube]): Int =
      if n < 0 then cubes.map(_.closest).min
      else
  ...
  def main(args: Array[String]): Unit =
    ...
    // val file = os.pwd / "2018" / "23" / "23.test.input.txt"
    val file = os.pwd / "2018" / "23" / "23.input.txt"
    val data = os.read.lines(file)
    println(part1(data))
    println(part2(data))
