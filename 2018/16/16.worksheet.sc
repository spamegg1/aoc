...
Addition:
  addr (add register) stores into register C result of adding register A and register B.
...
Before the instruction is executed, register 0 has value 3, register 1 has value 2,
and registers 2 and 3 have value 1. After the instruction is executed,
...
but only three of them behave in a way that would cause the result shown in the sample:
...
object Testing:
  private lazy val lines = os.read.lines(os.pwd / "2018" / "16" / "16.test.input.txt")
...
object Main:
  private lazy val lines = os.read.lines(os.pwd / "2018" / "16" / "16.input.txt")
  lazy val result1 = Solving.solve1(lines)
  lazy val result2 = Solving.solve2(lines)
// Main.result1 // part 1: ???
// Main.result2 // part 2: ???
