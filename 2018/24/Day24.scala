...
object Day24:
  ...
  case class Group(army: Army, attribute: Attribute, defense: Defense, attack: Attack)(
      var units: Int
  ):
  ...
  def fight(groups: Seq[Group]): (Army, Int) =
    ...
    targets.toSeq.sortBy((target, attacker) => -attacker.attribute.initiative).foreach {
      (target, attacker) =>
        target.units =
  ...
  def main(args: Array[String]): Unit =
    ...
    val data = os.read.lines(os.pwd / "2018" / "24" / "24.test.input.txt")
    // val data = os.read.lines(os.pwd / "2018" / "24" / "24.input.txt")
    println(part1(data))
    println(part2(data))
