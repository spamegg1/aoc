...
object Day11:
    ...
    first.result match
      case Output(color) =>
        ...
        paint(
          ...
          panels.updated(position, color)
        )
      case _ => panels
  end paint

  def part1(memory: Seq[Long]): Long =
    paint(IntCode(memory), Point(0, -1), Point(0, 0), Map().withDefaultValue(0L)).size

  def part2(memory: Seq[Long]): Map[Point, Long] = paint(
    IntCode(memory),
    Point(0, -1),
    Point(0, 0),
    Map(Point(0, 0) -> 1L).withDefaultValue(0L)
  )
  ...
  def main(args: Array[String]): Unit =
    ...
    for y <- minY to maxY do
      println()
      for x <- minX to maxX do print(if panels(Point(x, y)) == 1 then '#' else ' ')
  end main
