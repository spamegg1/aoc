...
object Day09:
  ...
  case class IntCode(
    ...
    private def write(offset: Int, value: Long): Map[Long, Long] =
      (memory(ip) / powers(offset)) % 10 match
    ...
    def next: IntCode = memory(ip) % 100 match
      case 1 =>
        copy(ip = ip + 4, memory = write(3, read(1) + read(2)), result = Running) // Add
      case 2 =>
        copy(
          ip = ip + 4,
      ...
      case 5 =>
        copy(
          ip = if read(1) != 0 then read(2) else ip + 3,
          result = Running
        ) // Jump if true
      case 6 =>
        copy(
      ...
      case 9 =>
        copy(
          ip = ip + 2,
          relativeBase = relativeBase + read(1),
          result = Running
        ) // Relative base
    ...
    def nextOutput: IntCode =
      Iterator.iterate(next)(_.next).dropWhile(_.result == Running).next()
  ...
  def main(args: Array[String]): Unit =
    val data = io.Source
      .fromResource("AdventOfCode2019/Day09.txt")
      .mkString
      .trim
      .split(",")
      .map(_.toLong)
