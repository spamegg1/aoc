...
object Day13:
  object Ansi:
    ...
    def echo(score: Long, blocks: Long, tiles: Map[Point, Long]): Unit =
      ...
      for y <- 0 until 20 do
        for x <- 0 until 44 do buffer += sprites(tiles.getOrElse(Point(x, y), 0))
        buffer += "\n"
      buffer += Reset
  ...
  def part2(memory: Seq[Long]): Long =
    ...
    def play(
      ...
      val (nextScore, nextPaddle, nextBall, nextTiles) =
        step(score, paddle, ball, tiles, output)
      ...
      if nextBlocks == 0 then nextScore
      else play(finalComputer, 6, nextScore, nextPaddle, nextBall, nextTiles)
    end play
  ...
  def main(args: Array[String]): Unit =
    // val file = os.pwd / "2019" / "13" / "13.input.txt"
    val file = os.pwd / "2019" / "13" / "13.input.2.txt"
    val line = os.read.lines(file).head
    val data = line.split(",").map(_.toLong).toSeq
    println(part1(data))
    println(part2(data))
