...
object Day17:
  ...
  def part1(memory: Seq[Long]): Int =
    ...
    val points =
      for
        ...
        x <- 0 until input.head.size
        if input(y)(x) == '#'
      yield Point(x, y)
    val neighbours = Seq(Point(0, -1), Point(0, 1), Point(-1, 0), Point(1, 0))
    points.foldLeft(0): (total, next) =>
      if neighbours.map(next + _).forall(points.contains)
      then total + (next.x * next.y)
      else total
  ...
  def part2(memory: Seq[Long]): Long =
    ...
    val videoFeed   = "n"
    val input = Seq(mainRoutine, functionA, functionB, functionC, videoFeed)
      .map(_ + "\n")
      .flatten
      .map(_.toLong)
  ...
  def main(args: Array[String]): Unit =
    val file = os.pwd / "2019" / "17" / "17.input.txt"
    val line = os.read.lines(file).head
    val data = line // io.Source.fromResource("AdventOfCode2019/Day17.txt")
      .mkString.trim
      .split(",")
      .map(_.toLong)
      .toSeq
