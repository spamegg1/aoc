 ...
 */
object DataDefs:
  extension (l: Long) def take16 = l.toBinaryString.reverse.take(16)

  object Gen:
    private val divisor = 2147483647L

  case class Gen(start: Long, multiplier: Long, value: Long, divBy: Long):
    private lazy val nextVal = (value * multiplier) % Gen.divisor
    def next1: Gen = copy(value = nextVal)
    def next2: Gen = if nextVal % divBy == 0 then next1 else next1.next2
    def compare(that: Gen) = value.take16 == that.value.take16

  case class Judge(genA: Gen, genB: Gen, count: Long):
    private def next(nextFun: Gen => Gen) =
      val (nextA, nextB) = (nextFun(genA), nextFun(genB))
      Judge(nextA, nextB, count + (if nextA.compare(nextB) then 1L else 0L))
    lazy val next1 = next(_.next1)
    lazy val next2 = next(_.next2)

object Solving:
  import DataDefs.*

  private def solve(nextFun: Judge => Judge)(judge: Judge)(pairs: Int) =
    var j = judge
    for _ <- 0 until pairs do j = nextFun(j)
    j.count
