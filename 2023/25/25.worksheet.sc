...
object DataDefs:
  ...
  case class Node(component: Component, connections: List[Component])
  type MyEdge = UnDiEdge[Component]
  type MyGraph = Graph[Component, MyEdge]

object Parsing:
  import DataDefs.*

  private def parseComponent(line: String): Node = line match
    case s"$component: $list" => Node(component, list.split(" ").toList)

  def parseComponents(lines: Seq[String]): Seq[Node] = lines.map(parseComponent)

object Solving:
  import DataDefs.*

  def populate(nodes: Seq[Node]): MyGraph =
    val graph: MyGraph = Graph.empty
    for
      node <- nodes
      connection <- node.connections
    do graph += node.component ~ connection
    graph

  def crossingEdges(subgraph: MyGraph)(graph: MyGraph) = graph.edges
    .filter: edge =>
      (!subgraph.contains(edge.source) && subgraph.contains(edge.target)) ||
        (subgraph.contains(edge.source) && !subgraph.contains(edge.target))
    .map(_.outer)

  @annotation.tailrec
  private def helper(graph: MyGraph)(subgraph: MyGraph)(
      crossing: Set[MyEdge]
  ): Set[MyEdge] =
    if crossing.size == 3 then crossing
    else helper(graph)(subgraph)(crossing)

  def solve1(lines: Seq[String]): Long =
    val nodes = Parsing.parseComponents(lines)
    val graph = populate(nodes)
    val subgraph: MyGraph = Graph.empty
    val threeEdges = helper(graph)(subgraph)(Set[MyEdge]())
    graph --= threeEdges
