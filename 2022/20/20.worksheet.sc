...
so if you can figure out where that is,
...
Unfortunately, the file is encrypted -
just in case the device were to fall into the wrong hands.

Maybe you can decrypt it?

When you were still back at the camp,
you overheard some Elves talking about coordinate file encryption.
The main operation involved in decrypting the file is called mixing.

The encrypted file is a list of numbers.
To mix the file, move each number forward or backward
in the file a number of positions equal to the value of
the number being moved. The list is circular, so moving a
number off one end of the list wraps back around to the other
...
Numbers moving around during the mixing process do not change the order
...
Then, the grove coordinates can be found by looking at the
...
While you ponder the mysteries of Elf encryption,
...
Here, the 1000th number after 0 is 811589153,
the 2000th is 2434767459, and the 3000th is -1623178306;
 ...
 */
object DataDefs:
  extension (n: Int) def clamp(size: Int) = n + (if n <= 0 then size else 0)

  case class Num(initIndex: Int, value: Int, curIndex: Int):
    def update(newIndex: Int): Num = copy(curIndex = newIndex)

  case class State(nums: Seq[Num])(using size: Int):
    def rangeChange(curIndex: Int, newIndex: Int): (Range, Int) =
      if newIndex < curIndex
      then (newIndex until curIndex, 1)
      else (curIndex + 1 to newIndex, -1)

    def next(initIndex: Int, value: Int): State =
      val theNum = nums.find(_.initIndex == initIndex).get
      val newIndex = ((theNum.curIndex + theNum.value) % (size - 1)).clamp(size - 1)
      val (range, change) = rangeChange(theNum.curIndex, newIndex)
      val newNums = nums.map: num =>
        if num == theNum then theNum.update(newIndex)
        else if range contains num.curIndex then num.update(num.curIndex + change)
        else num
      State(newNums)

    lazy val curIndexofZero = nums.find(_.value == 0).get.curIndex

    def afterZero(by: Int): Int =
      val index = (by + curIndexofZero) % size
      nums.find(_.curIndex == index).get.value

    override def toString = nums
      .map(num => (num.curIndex, num.value))
      .sortBy(_._1)
      .map(_._2)
      .mkString(", ")
...
object Parsing:
  import DataDefs.*
  def parse(lines: Seq[String]): Seq[(Int, Int)] = lines.map(_.toInt).zipWithIndex
  def parseState(ints: Seq[(Int, Int)]): State =
    State(ints.map((value, index) => Num(index, value, index)))(using ints.size)

object Solving:
  import DataDefs.*

  def solve1(lines: Seq[String]) =
    val ints = Parsing.parse(lines)
    var state = Parsing.parseState(ints)
    for (value, index) <- ints do state = state.next(index, value)
    Seq(1000, 2000, 3000).map(state.afterZero)
...
// Test.res1 // part 1: 4 -3 2
// Test.res2 // part 2:
// Test.start //  1, 2, -3, 3, -2, 0, 4 start
...
object Main:
  ...
  lazy val res1 = Solving.solve1(lines)
  lazy val res2 = Solving.solve2(lines)
// Main.res1 // part 1: 2034 5418 -1490 = 5962
// Main.res2 // part 2:
