...
When this mountain was a volcano, the ambient temperature was
more reasonable; now, it's so cold that the fuel
...
"That's right, just supply the fuel amount to the-- oh,
for more than one burner? No problem, you just need to add together
our Special Numeral-Analogue Fuel Units.
...
normal numbers have a ones place, a tens place, a hundreds place, and so on,
...
a twenty-fives place, a one-hundred-and-twenty-fives place, and so on. It's that easy!"

You ask why some of the digits look like - or = instead of "digits".

"You know, I never did ask the engineers why they did that.
Instead of using digits four through zero,
the digits are 2, 1, 0, minus (written -),
and double-minus (written =).
Minus is worth -1, and double-minus is worth -2."

"So, because ten (in normal numbers) is two fives and no ones,
in SNAFU it is written 20. Since eight (in normal numbers) is
...
so I'll transmit our handy SNAFU brochure to you now. Did you need anything else?"

You ask if the fuel will even work in these temperatures.

"Wait, it's how cold? There's no way the fuel - or any fuel -
would work in those conditions!
...
The SNAFU brochure contains a few more examples of
decimal ("normal") numbers and their SNAFU counterparts:
...
--- Part Two ---
The hot air balloons quickly carry you to the North Pole.
As soon as you land, most of the expedition is escorted
 ...
 */
object DataDefs:
  type BaseFive = String
  type Snafu = String

  extension (c: Char)
    def fromSnafu = c match
      case '-' => -1
      case '=' => -2
      case _   => c.asDigit

  extension (l: Long) def toBaseFive: BaseFive = java.lang.Long.toString(l, 5)

  extension (baseFive: String)
    def toSnafu: Snafu =
...
object Parsing:
  import DataDefs.*
  def parseLine(line: String) = line.reverse.map(_.fromSnafu).zipWithIndex
  def parse(lines: Seq[String]) = lines map parseLine

object Solving:
  import DataDefs.*

  def toDecimal(line: Seq[(Int, Int)]): Double = line
    .map: (digit, power) =>
      math.pow(5.0, power.toDouble) * digit.toDouble
    .sum
