...
You pull it out of your pack, but the amount of water slowly draining
...
For example, consider this larger program: 10.test.input.txt
...
there is no such thing as "vertical position": if the sprite's horizontal position
...
object DataDefs:
  ...
  case class State(xReg: Int, cycle: Int):
    lazy val signalStrength = xReg * cycle

    def next(op: Op): State = op.cycles match
      case 0 => State(xReg + op.value, cycle + 1)
      case _ => State(xReg, cycle + 1)

object Parsing:
  import DataDefs.*, Op.*

  def parseLine(line: String): Seq[Op] = line match
    case s"addx $value" => Seq(Op(value.toInt, 1), Op(value.toInt, 0))
    case s"noop"        => Seq(Op(0, 0))
...
object Solving:
  ...
  private val start = List(State(1, 1))

  def solve1(lines: Seq[String]) = Parsing
    .parse(lines)
    .view
    .foldLeft(start)((states, op) => states.head.next(op) :: states)
...
object Testing:
  private lazy val lines = os.read.lines(os.pwd / "2022" / "10" / "10.test.input.txt")
  lazy val result1 = Solving.solve1(lines)
  lazy val result2 = Solving.solve2(lines)
// Testing.result1 // part 1: 13140
// Testing.result2 // part 2:

object Main:
  private lazy val lines = os.read.lines(os.pwd / "2022" / "10" / "10.input.txt")
  lazy val result1 = Solving.solve1(lines)
  lazy val result2 = Solving.solve2(lines)
// Main.result1 // part 1: 17380
// Main.result2 // part 2:
