...
To pass through the force field, you have to enter a password;
...
The monkeys give you notes that they took when they
last saw the password entered (your puzzle input).
...
It is comprised of a set of open tiles (on which you can move, drawn .)
...
It consists of alternating numbers and letters:
  A number indicates the number of tiles to move in the direction you are facing.
    If you run into a wall, you stop moving forward and continue with next instruction.
...
So, a path like 10R5 means "go forward 10 tiles,
then turn clockwise 90 degrees, then go forward 5 tiles".
...
you should instead look in the direction opposite of your
current facing as far as you can until you find the opposite
edge of the board, then reappear there.

For example, if you are at A and facing to the right,
the tile in front of you is marked B; if you are at C and facing down,
the tile in front of you is marked D:
...
you need to determine numbers for your final row, column,
and facing as your final position appears from the perspective
of the original map. Rows start from 1 at the top and count downward;
columns start from 1 at the left and count rightward.
(In the above example, row 1, column 1 refers to the empty space
with no tile on it in the top-left corner.) Facing is 0 for right
(>), 1 for down (v), 2 for left (<), and 3 for up (^).
The final password is the sum of 1000 times the row, 4 times the column, and the facing.

In the above example, the final row is 6, the final column is 8, and
the final facing is 0. So, the final password is 1000 * 6 + 4 * 8 + 0: 6032.
...
object Solving:
  ...
  def solve1(lines: Seq[String])(path: String) = 0L
  def solve2(lines: Seq[String])(path: String) = 0L

object Testing:
  private lazy val lines = os.read.lines(os.pwd / "2022" / "22" / "22.test.input.txt")
  lazy val result1 = Solving.solve1(lines)("10R5L5R10L4R5L5")
  lazy val result2 = Solving.solve2(lines)
// Testing.result1 // part 1: 6032
// Testing.result2 // part 2:

object Main:
  private lazy val lines = os.read.lines(os.pwd / "2022" / "22" / "22.input.txt")
  private lazy val path = os.read.lines(os.pwd / "2022" / "22" / "22.input.2.txt").head
  lazy val result1 = Solving.solve1(lines)(path)
  lazy val result2 = Solving.solve2(lines)(path)
// Main.result1 // part 1:
// Main.result2 // part 2:
