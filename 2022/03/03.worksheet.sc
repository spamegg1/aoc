...
object DataDefs:
  ...
  case class Rucksack(stuff: String):
    private lazy val halfSize = stuff.length / 2
    private lazy val first: Compartment = stuff.take(halfSize)
    private lazy val second: Compartment = stuff.drop(halfSize)
    private lazy val findCommon = first.toSet.intersect(second.toSet).headOption
    lazy val priority = findCommon.map(char => priorities(char)) // part 1

  case class Group(rucksacks: Seq[Rucksack]):
    private lazy val first: Rucksack = rucksacks.head
    private lazy val second: Rucksack = rucksacks.tail.head
    private lazy val third: Rucksack = rucksacks.last
    private lazy val findCommon = first.stuff.toSet
      .intersect(second.stuff.toSet)
      .intersect(third.stuff.toSet)
      .headOption
    lazy val priority = findCommon.map(char => priorities(char))

object Parsing:
  import DataDefs.*

  private def parseLine(line: String): Rucksack = Rucksack(line)
