...
for you and the elephants. The ground is rumbling almost continuously now,
...
The pattern above means that the jets will push a falling rock right, then right,
then right, then left, then left, then right, and so on.
...
an already-fallen rock, the falling rock stops where it is
(having landed on something) and a new rock immediately begins falling.

Drawing falling rocks with @ and stopped rocks with #,
...
To prove to the elephants your simulation is accurate,
they want to know how tall the tower will get after 2022 rocks have stopped
...
object Testing:
  private lazy val line = os.read.lines(os.pwd / "17.test.input.txt").head
...
object Main:
  private lazy val line = os.read.lines(os.pwd / "17.input.txt").head
  lazy val result1 = Solving.solve1(2022)(line)
  lazy val result2 = Solving.solve2(2022)(line)
// Main.result1 // part 1:
// Main.result2 // part 2:
