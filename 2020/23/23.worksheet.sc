...
Each move, the crab does the following actions:
  ...
  The crab selects a destination cup: the cup with a label equal to the current
    cup's label minus one. If this would select one of the cups that was
    just picked up, the crab will keep subtracting one until it finds a cup
    that wasn't just picked up. If at any point in this process the value goes
    below the lowest value on any cup's label, it wraps around to the
  ...
  The crab selects a new current cup: the cup
...
appear moving clockwise around the circle; the current cup is marked with ( ).
